...
exports.CrisisInterventionService = void 0;
const database_1 = require("../config/database");
const types_1 = require("../types");
class CrisisInterventionService {
    static async logCrisisEvent(userId, crisisLevel, triggerSource, detectedKeywords, userMessage) {
        try {
            await database_1.prisma.crisisEvent.create({
                data: {
                    userId,
                    crisisLevel,
                    triggerSource,
            ...
            throw new types_1.ApiError(500, 'Failed to log crisis event');
        }
    }
    static async recordIntervention(crisisEventId, interventionType, resourcesProvided, professionalContactMade = false) {
        try {
            await database_1.prisma.crisisEvent.update({
            ...
            throw new types_1.ApiError(500, 'Failed to record intervention');
        }
    }
    static async getCrisisHistory(userId) {
        try {
            const crisisEvents = await database_1.prisma.crisisEvent.findMany({
                where: { userId },
            ...
            throw new types_1.ApiError(500, 'Failed to get crisis history');
        }
    }
    static async getCrisisResources(language = 'id') {
    ...
    static async triggerEmergencyProtocol(userId, crisisLevel, location) {
        try {
            // Log the emergency protocol trigger
            await this.logCrisisEvent(userId, crisisLevel, 'emergency_protocol', ['emergency_trigger'], 'Emergency protocol activated');
            ...
            throw new types_1.ApiError(500, 'Failed to trigger emergency protocol');
        }
    }
}
exports.CrisisInterventionService = CrisisInterventionService;
//# sourceMappingURL=crisisInterventionService.js.map